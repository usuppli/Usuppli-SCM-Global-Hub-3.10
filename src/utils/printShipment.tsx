...
export const printShipment = (shipment: Shipment) => {
  ...
  const html = `
    ...
    <html>
    <head>
      <title>Manifest_${shipmentId}</title>
      <style>
        ...
        .title { font-size: 32px; font-weight: 800; color: #003d5b; margin: 0; line-height: 1; text-transform: uppercase; }
        .ref { font-size: 14px; font-weight: 700; color: #64748b; margin-top: 4px; font-family: monospace; }

        .info-grid { display: grid; grid-template-cols: repeat(4, 1fr); gap: 20px; margin-bottom: 30px; border-top: 2px solid #f1f5f9; padding-top: 20px; }
      ...
      <div class="container">
        <div class="header">
          ...
          <div class="header-right">
            <h1 class="title">Shipping Manifest</h1>
            <div class="ref">MANIFEST REF: ${shipmentId}</div>
          </div>
        </div>
        ...
        <div class="info-grid">
          <div class="grid-col">
            <h4>Origin</h4>
            <p>${shipment.origin || 'Pending'}</p>
          </div>
          <div class="grid-col">
            <h4>Destination</h4>
            <p>${shipment.destination || 'Pending'}</p>
          </div>
          <div class="grid-col">
            <h4>Logistics Provider</h4>
            <p>${shipment.carrier || 'N/A'}</p>
            <p>Method: ${shipment.method || 'N/A'}</p>
          </div>
          <div class="grid-col">
            <h4>Tracking Information</h4>
            <p>${shipment.trackingNumber || 'N/A'}</p>
            <p>ETA: ${shipment.eta || 'TBD'}</p>
          </div>
        </div>
        ...
        <table>
          <thead>
            <tr>
              <th>Container / Lot ID</th>
              <th>Transport Type</th>
              <th class="text-right">Estimated Weight</th>
              <th class="text-right">Voyage Status</th>
            </tr>
          </thead>
          <tbody>
            <tr>
              <td style="font-family: monospace;">${shipment.trackingNumber || 'BATCH-001'}</td>
              <td>${shipment.method || 'Standard'}</td>
              <td class="text-right">As Per BL</td>
              <td class="text-right">${shipment.status || 'Active'}</td>
            </tr>
          </tbody>
        </table>
        ...
        <div class="footer">
          <div class="signature-block">
            ...
            <div class="signature-label">Officer In Charge (OIC)</div>
          </div>
        </div>
      </div>
      <script>
